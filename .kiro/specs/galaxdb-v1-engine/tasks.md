...
- [x] 40. Implement deployment modes — galaxdb-server, galaxdb-embedded (Req 35)
  - [x] 40.1 Implement standalone server binary: tokio main, bind wire protocol + HTTP observability, spawn sidecar, graceful shutdown on SIGTERM/SIGINT <!-- Real: `galaxdb-server/src/main.rs` — tokio main, `galaxdb_observe::init_logging()` for JSON logging, `--port`/`--data-dir`/`--sidecar`/`--model` CLI args, `galaxdb_server::start()` binds wire protocol, `ServerConfig::sidecar_binary`/`model_id` fields wire `Database::open_with_sidecar` when configured. Graceful shutdown: `tokio::select!` on SIGTERM/SIGINT (unix) or Ctrl-C (Windows) with 5 s drain window. -->
