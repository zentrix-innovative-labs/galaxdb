...
- [x] 40. Implement deployment modes — galaxdb-server, galaxdb-embedded (Req 35)
  - [x] 40.1 Implement standalone server binary: tokio main, bind wire protocol + HTTP observability, spawn sidecar, graceful shutdown on SIGTERM/SIGINT <!-- Real: `galaxdb-server/src/main.rs` — tokio main, `galaxdb_observe::init_logging()` for JSON logging, `--port`/`--data-dir`/`--sidecar`/`--model`/`--observe-port` CLI args, `galaxdb_observe::start_http()` binds HTTP /health + /metrics on port 9090 (configurable), `galaxdb_server::start()` binds wire protocol, `ServerConfig::sidecar_binary`/`model_id` fields wire `Database::open_with_sidecar` when configured. Graceful shutdown: `tokio::select!` on SIGTERM/SIGINT (unix) or Ctrl-C (Windows) with 5 s drain window. Test: `http_observability_starts_alongside_wire_server` confirms /health returns 200 JSON and /metrics returns Prometheus text format. -->
  - [x] 40.2 Implement embedded mode: PyO3 `Database` class with `new(path)`, `execute(sql)`, `training_dataset(tag)` methods <!-- Real: `galaxdb-python/src/lib.rs` — `Database(path)`, `execute(sql)`, `training_dataset(tag)`, `create_training_snapshot(name, seed)`. All methods drive the real `galaxdb-embedded::Database`. -->
  - [x] 40.3 Implement platform-specific I/O backend selection: io_uring on Linux, tokio on macOS/Windows <!-- Real: `galaxdb-io::select_scheduler()` picks `IoUringScheduler` on Linux 5.10+ with io_uring available, `TokioScheduler` on macOS/Windows or when `GALAXDB_IO_BACKEND=tokio`. `Engine::new` calls `select_scheduler()` at startup. Tests: `detect_backend_defaults_to_tokio_on_macos`, `detect_backend_respects_env_var_tokio`. -->
