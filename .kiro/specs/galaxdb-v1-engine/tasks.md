...
- [ ] 38. Implement observability — galaxdb-observe (Req 28)
  - [x] 38.1 Implement embedded HTTP server (axum) with `/health` endpoint returning JSON status <!-- Real: `galaxdb_observe::start_http` spawns an axum `Router` on a caller-supplied addr; `/health` returns `HealthResponse { status: "ok", version }` as JSON. Test: `http_health_returns_ok_json` binds port 0, GETs via `reqwest`, asserts body. -->
  ...
  - [x] 38.3 Register all metrics: buffer_pool_hot_set_usage, buffer_pool_scan_buffer_usage, embedding_queue_depth, embedding_backlog_depth, checkpoint_last_duration_ms, compaction_pending_bytes, wal_write_latency_us, hnsw_recall_estimate, connections_active, disk_full, sidecar_status <!-- Real: `galaxdb_observe::Metrics` struct holds an IntGauge/IntCounter handle for every spec-listed metric plus `queries_total`; `metrics()` lazily registers all 12 against the default registry on first call. Test: `all_spec_metrics_register` scrapes the registry and asserts every name is present. Scope note: wiring from owning crates (buffer pool, compactor, sidecar, HNSW) happens as each owner updates its handle via `metrics()`; today only `disk_full` is wired live by `galaxdb-storage::disk_full`, others start at zero until their owners call through. -->
  - [x] 38.4 Implement structured JSON logging via tracing-subscriber with configurable level (GALAXDB_LOG_LEVEL env var) <!-- Real: `galaxdb_observe::init_logging` installs a `tracing_subscriber::fmt().json()` subscriber with `EnvFilter::try_from_env("GALAXDB_LOG_LEVEL")` falling back to `info`; includes thread id, file, line. Idempotent (set_global_default returns err on second call, treated as OK). -->
  ...
  - [x] 38.6 Implement SQL commenter format for trace context in wire protocol <!-- Real: `extract_traceparent_from_sql` + `append_traceparent_to_sql` parse/render `/* traceparent='00-<32hex>-<16hex>-<2hex>' */` suffixes. Tests: `sql_commenter_extract`, `sql_commenter_append` (round-trip through `Traceparent`). -->
  - [x] 38.7 Write tests: /health returns correct status, /metrics returns valid Prometheus format, trace spans created for query execution <!-- 11 observe crate tests: HTTP /health JSON body, /metrics Prometheus format + content-type, Traceparent round-trip + invalid rejection + unsampled flag, SQL commenter extract/append, metric registry stability + registration + all spec metrics present. Full span coverage (38.5) is the remaining piece. -->
